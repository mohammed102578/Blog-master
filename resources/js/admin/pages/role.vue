...
export default {
	...
	methods : {
		async add(){
			...
			const res = await this.callApi('post', '/create_role', this.data)
		...
		async edit(){
			...
			const res = await this.callApi('post', '/edit_role', this.editData)
			if(res.status===200){
		...
		async deleteTag(){
			this.isDeleing = true
			const res = await this.callApi('post', 'delete_role', this.deleteItem)
			if(res.status===200){
	...
	async created(){
		const res = await this.callApi('get', 'get_roles')
		if(res.status==200){
			this.roles = res.data
		}else{
			this.swr()
